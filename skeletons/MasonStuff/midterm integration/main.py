...
def format_diagnosis_text(plant, classification, management, probabilities):
    ...
    if probabilities:
        ...
        lines.append(
            ...
            f"{top_disease} ({round(top_prob * 100, 2)}% confidence)."
        )
    else:
    ...
    if probabilities and len(probabilities) > 1:
        ...
        for disease, prob in probabilities[1:]:
            lines.append(f"• {disease} ({round(prob * 100, 2)}%)")

    return "\n".join(lines)  # ← was missing


def build_diagnosis_from_results(results):
    """Converts engine results list into format_diagnosis_text arguments and calls it."""
    # Filter out negative probabilities and sort descending
    valid = sorted(
        [r for r in results if r["probability"] > 0],
    ...
    top = valid[0]
    probabilities = [(r["disease"], r["probability"]) for r in valid]

    return format_diagnosis_text(
        plant=top["plant"],
...
# -------------------------
#flag
#need to get plant name and send confidence scores with symptoms
...
def diagnose_image():
    ...
    try:
       ...
       # symptom_names = [s["symptom_name"] for s in predicted_symptoms]
        ...
        symptoms = {
        s["symptom_name"]: round(float(s["probability_percent"]) / 100, 2)
        for s in predicted_symptoms
        }

        print("🧠 Symptoms sent to expert system:", symptoms)

        # 🔥 Run expert system
        engine = MultiSymptom(kb_data)
        engine.reset()
        engine.declare(symptomatic(name=plant, symptoms=symptoms,temperature=None, humidity=None,lengthOfW=None ))
        engine.run()
        results = engine.results  # list of dicts
        print(build_diagnosis_from_results(results))
        return jsonify(build_diagnosis_from_results(results)),200
...
def diagnose():
    ...
    zipcode="46502"
    #plant = ("Tomato")
    #symptoms = ["Sunscald","Sunken","Spots","Blister","Greasy","Halo"]
    DEFAULT_SYMPTOM_CONFIDENCE = 0.80
    symptoms = {symptom: DEFAULT_SYMPTOM_CONFIDENCE for symptom in symptoms}
    ...
    if zipcode is None or zipcode=="":
        ...
        ["python", "weatherAPI.py", zipcode],  # ← zip code here
        capture_output=True,
        text=True
        )
    ...
    try:
        engine = MultiSymptom(kb_data)
        engine.reset()
        engine.declare(symptomatic(name=plant, symptoms=symptoms,temperature=temp, humidity=humid,lengthOfW=lengthOfW ))
        engine.run()
        results = engine.results  # list of dicts
        print(build_diagnosis_from_results(results))
        return jsonify(build_diagnosis_from_results(results)),200
