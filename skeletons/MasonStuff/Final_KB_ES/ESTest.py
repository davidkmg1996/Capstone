...
class PlantDiseaseEngine(KnowledgeEngine):
        ...
        for disease_name, entry_list in self.kb["diseases"].items():
            for entry in entry_list:
                ...
                for symptom, confidence in symptoms.items():
                    weight = 0.3  # weight per symptom
                    weighted_cf = weight * confidence  # scale by user confidence
    
                    if symptom in disease_symptoms:
                        cf_symptoms = combine_cf(cf_symptoms, weighted_cf)
                    else:
                        cf_symptoms = combine_cf(cf_symptoms, -weighted_cf)
    ...
    def check_keySymptom(self, candidate, disease, plant, cf_running,
        ...
        matched_cf_values = [userSymptoms[symptom] for symptom in matched_key]

        print(matched_key)
        print(matched_cf_values)
        if matched_key:
            cfKey=0.7*matched_cf_values[0]
